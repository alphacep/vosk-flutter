...
class VoskFlutterPlugin {
  ...
  static Future<void> initModel(ByteData modelZip) async {
    File model = await _writeToFile(modelZip, (await getTemporaryDirectory()).path + '/model_temp.zip');
    String modelPath = await _decompressModel(model);
  ...
  static Future<String> _decompressModel(File zipModelFile) async {

    // Decode the Zip file
    final archive = ZipDecoder().decodeBytes(zipModelFile.readAsBytesSync());
